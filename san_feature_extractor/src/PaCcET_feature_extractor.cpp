...
//   odomPose1.header.frame_id = "/robot_1/odom";
//   odomPose1.header.stamp = ros::Time();
//   odomPose1.pose = msg->pose.pose;
  
//   tf::TransformListener listener;
//   listener.waitForTransform("/robot_1/odom", "/robot_0/odom", ros::Time(0), ros::Duration(10.0));
//   geometry_msgs::PoseStamped mapRobot1Pose;
//   try{
//         listener.transformPose("robot_0/odom", odomPose1, mapRobot1Pose);
//         globalPose1 = mapRobot1Pose;
...
void robot1Callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  odomPose1.pose = msg->pose.pose;
  tf::poseMsgToTF(odomPose1.pose, transform);
  tf::StampedTransform tx(transform, odomPose1.header.stamp, "/map", "/robot_1/base_link");
  ROS_INFO("I heard: [%s]", msg->header.frame_id.c_str());
  br.sendTransform(tx);
...
int main(int argc, char **argv)
{
   ...
   */
  ros::init(argc, argv, "map_base_link");
